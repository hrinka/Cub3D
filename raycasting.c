...
/*   Updated: 2024/05/31 23:57:51 by hrinka           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int id_ray, mlx_image_t *img)
{
	...
	while (ystart < yend)
	{
		data->render.texture_offset_y = (((float)ystart - (float)data->wall_start)
				/ (float)height_wall) * img->height;
		if (ystart >= 0 && ystart < HEIGHT_WIN)
			mlx_put_pixel(data->map.img, xstart, (int)ystart,
				get_texel(img, data->render.texture_offset_x, data->texture_offset_y));
		ystart += 1;
	}
}
...
void	check_ray_draw_down(t_cub3d *data, float ray_angle)
{
	...
	data->render.next_hor_inters_x = data->render.hores_inters_x + ((data->next_hor_inters_y
	...
	while (((int)(data->render.hores_inters_x / data->map.size_shape)) < data->width_map
		&& ((int)(data->render.hores_inters_y / data->map.size_shape)) < data->height_map
...
{
	data->render.vertcl_inters_x = floor((data->map.px / data->map.size_shape) + 1)
		* data->map.size_shape;
	data->render.vertcl_inters_y = data->map.py - ((data->map.px - data->vertcl_inters_x)
			* tan(to_rad(ray_angle)));
	data->render.next_ver_inters_x = data->render.vertcl_inters_x + data->map.size_shape;
	data->render.next_ver_inters_y = data->render.vertcl_inters_y - ((data->vertcl_inters_x
				- data->render.next_ver_inters_x) * tan(to_rad(ray_angle)));
	data->render.step_ver_x = data->map.size_shape;
	data->render.step_ver_y = data->render.next_ver_inters_y - data->vertcl_inters_y;
	while (((int)(data->render.vertcl_inters_x / data->map.size_shape)) < data->width_map
		&& ((int)(data->render.vertcl_inters_y / data->map.size_shape)) < data->height_map
		&& data->render.vertcl_inters_x >= 0 && data->render.vertcl_inters_y >= 0
		&& data->map.height_map[(int)(data->render.vertcl_inters_y
			/ data->map.size_shape)][(int)(data->render.vertcl_inters_x
			/ data->map.size_shape)] != '1')
	{
