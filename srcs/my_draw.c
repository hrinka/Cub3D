...
/*   Updated: 2024/06/08 17:05:53 by hirosuzu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     double wall_x = ray->side == 0 ? game->player.pos_y + ray->perp_wall_dist * ray->ray_dir_y
//                                    : game->player.pos_x + ray->perp_wall_dist * ray->ray_dir_x;
//     wall_x -= floor(wall_x);

//     int tex_num = ray->tex_num;  // テクスチャ番号を取得
//     mlx_texture_t *texture = game->textures[tex_num];  // テクスチャを取得
//     if (!texture) {
//         fprintf(stderr, "Failed to get texture\n");
//         return;
//     }
...
//         int tex_y = ((d * texture->height) / line_height) / 256;
//         uint32_t color = get_texel(texture, tex_x, tex_y);
